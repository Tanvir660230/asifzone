...
function readLocalDraft(): Partial<CreateProductInput> | null {
  try {
    const raw = window.localStorage.getItem(DRAFT_STORAGE_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as { values: Partial<CreateProductInput> };
    return parsed.values ?? null;
  } catch {
    return null;
  }
}
function writeLocalDraft(values: Partial<CreateProductInput>) {
  try {
    window.localStorage.setItem(DRAFT_STORAGE_KEY, JSON.stringify({ values, savedAt: new Date().toISOString() }));
  } catch {
...
function renderStep(
  id: WizardStepId,
  state: WizardState,
  extra: { categories: Category[]; relationNames: Record<string, string>; onRelationNames: (added: Record<string, string>) => void },
) {
  switch (id) {
    case "basics":
...
export function ProductWizard({ categories, initial }: ProductWizardProps) {
  ...
  const [offeredRecovery, setOfferedRecovery] = useState(false);
  useEffect(() => {
    if (mode !== "new" || offeredRecovery) return;
    setOfferedRecovery(true);
    const saved = readLocalDraft();
    if (saved && (saved.name || saved.categoryId)) {
      if (window.confirm(`Continue where you left off on "${saved.name || "a new product"}"?`)) {
  ...
  useEffect(() => {
    const sub = form.watch((values) => {
      ...
      debounceRef.current = setTimeout(() => {
        if (mode === "new") {
          writeLocalDraft(values as Partial<CreateProductInput>);
        } else if (initial) {
          setSaveState("saving");
          productsApi
            .updateProduct(initial.id, withPrunedAttributes(values as CreateProductInput))
            .then(() => {
              setSaveState("saved");
  ...
  async function handlePanelAction(status?: ProductStatus) {
    if (!initial) return;
    setBusy(true);
    try {
      const values = withPrunedAttributes(form.getValues());
      const { product } = await productsApi.updateProduct(initial.id, { ...values, ...(status ? { status } : {}) });
  ...
  return (
    ...
    <div className="min-w-0 space-y-6">
      ...
      <div className="rounded-xl border border-ink-100 bg-white p-5">{renderStep(currentStepId, wizardState, { categories, relationNames, onRelationNames: (added) => setRelationNames((prev) => ({ ...prev, ...added })) })}</div>
